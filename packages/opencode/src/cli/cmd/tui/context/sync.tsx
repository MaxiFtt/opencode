            ...
            setStore(
              produce((draft) => {
                ...
                const fetched = (messages.data ?? []).map((message) => {
                  ...
                  const info = tracker.messages.has(message.info.id) && current ? current : message.info
                  const currentParts = draft.part[message.info.id] ?? []
                  const parts = message.parts.flatMap((part) => {
                    const existing = currentParts.find((item) => item.id === part.id)
                    if (tracker.parts.has(part.id)) return existing ? [existing] : []
                    if (
                      existing &&
                  ...
                  parts.push(...currentParts.filter((part) => tracker.parts.has(part.id) && !parts.some((item) => item.id === part.id)))
                  return { info, parts }
                })
                ...
                for (const message of messages.data ?? []) {
                  if (!visibleIDs.has(message.info.id)) delete draft.part[message.info.id]
                }
